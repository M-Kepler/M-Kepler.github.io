- [疑症（1）TCP 的三次握手、四次挥手](#疑症1tcp-的三次握手四次挥手)
- [疑症（2）TCP 连接的初始化序列号能否固定](#疑症2tcp-连接的初始化序列号能否固定)
- [疑症（3）初始化连接的 SYN 超时问题](#疑症3初始化连接的-syn-超时问题)
- [疑症（4） TCP 的 Peer 两端同时断开连接](#疑症4-tcp-的-peer-两端同时断开连接)
- [疑症（5）四次挥手能不能变成三次挥手呢？？](#疑症5四次挥手能不能变成三次挥手呢)
- [疑症（6） TCP 的头号疼症 TIME_WAIT 状态](#疑症6-tcp-的头号疼症-time_wait-状态)
  - [一、Peer 两端，哪一端会进入 TIME_WAIT 呢？为什么?](#一peer-两端哪一端会进入-time_wait-呢为什么)
  - [二、TIME_WAIT 状态是用来解决或避免什么问题呢？](#二time_wait-状态是用来解决或避免什么问题呢)
  - [三、TIME_WAIT 会带来哪些问题呢？](#三time_wait-会带来哪些问题呢)
  - [四、TIME_WAIT 的快速回收和重用](#四time_wait-的快速回收和重用)
  - [五、清掉 TIME_WAIT 的奇技怪巧](#五清掉-time_wait-的奇技怪巧)
- [疑症（7）TCP 的延迟确认机制](#疑症7tcp-的延迟确认机制)
- [疑症（8）TCP 的重传机制以及重传的超时计算](#疑症8tcp-的重传机制以及重传的超时计算)
  - [【1】TCP 的重传超时计算](#1tcp-的重传超时计算)
  - [【2】TCP 的重传机制](#2tcp-的重传机制)
- [疑症（9）TCP 的流量控制](#疑症9tcp-的流量控制)
  - [疑问 1）的解决](#疑问-1的解决)
  - [疑问 2）的解决](#疑问-2的解决)
  - [疑问点 3）的解决](#疑问点-3的解决)
- [疑症（10）TCP 的拥塞控制](#疑症10tcp-的拥塞控制)
  - [【1】慢热启动算法 – Slow Start](#1慢热启动算法--slow-start)
  - [【2】拥塞避免算法 – Congestion Avoidance](#2拥塞避免算法--congestion-avoidance)
  - [【3】快速重传 - Fast Retransimit 做的事情有](#3快速重传---fast-retransimit-做的事情有)
  - [【4】快速恢复算法 – Fast Recovery](#4快速恢复算法--fast-recovery)
  - [【1】接收端拥有大窗口的经典锯齿状](#1接收端拥有大窗口的经典锯齿状)
  - [【2】接收端拥有小窗口的直线状态](#2接收端拥有小窗口的直线状态)
  - [【3】两个直连网络端点间的满载状态下的直线状态](#3两个直连网络端点间的满载状态下的直线状态)
- [11. 附加题 1：P2P 理论上的加速比](#11-附加题-1p2p-理论上的加速比)
- [12. 附加题 2：系统调用 listen() 的 backlog 参数指的是什么](#12-附加题-2系统调用-listen-的-backlog-参数指的是什么)
  - [【1】SYN 半连接队列的作用](#1syn-半连接队列的作用)
  - [【2】accept 连接队列](#2accept-连接队列)

> 本文由 [简悦 SimpRead](http://ksria.com/simpread/) 转码， 原文地址 [mp.weixin.qq.com]((https://mp.weixin.qq.com/s/6LiZGMt2KRiIoMaLwx-lkQ)
...
![alt](https://mmbiz.qpic.cn/mmbiz_jpg/j3gficicyOvasF6pzMRVlOnbpMNic3XqHBPBQInmfEBgvdVd8WDPLodCiaCIHy1ib2mSLCMUGrf3pTric60MJeIzHlbg/640?wx_fmt=jpeg)TCP 三次握手、四次挥手时序图![alt](https://mmbiz.qpic.cn/mmbiz_png/j3gficicyOvasF6pzMRVlOnbpMNic3XqHBPtBWP3YibK8x5FYPxNxZO72LkOkBvguP62XVicz1z4naf6LgiaFJSw65ibA/640?wx_fmt=png) TCP 协议状态机
...
![alt](https://mmbiz.qpic.cn/mmbiz_png/j3gficicyOvasF6pzMRVlOnbpMNic3XqHBPyVEib5Jk4rGfS8WQ8CrhRLibCMDzVDAcUaJGwUMPnkRZTVmzE4C0Shjg/640?wx_fmt=png)
...
如果初始化序列号（缩写为 ISN：Inital Sequence Number）可以固定，我们来看看会出现什么问题。假设 ISN 固定是 1，Client 和 Server 建立好一条 TCP 连接后，Client 连续给 Server 发了 10 个包，这 10 个包不知怎么被链路上的路由器缓存了 (_路由器会毫无先兆地缓存或者丢弃任何的数据包_)，这个时候碰巧 Client 挂掉了，然后 Client 用同样的端口号重新连上 Server，Client 又连续给 Server 发了几个包，假设这个时候 Client 的序列号变成了 5。
...
![alt](https://mmbiz.qpic.cn/mmbiz_png/j3gficicyOvasF6pzMRVlOnbpMNic3XqHBPQQciaWgL8zAoNIVT1d2KA0ibwUWKIfGic6kDfpqa9MVXKV4keh765pfnA/640?wx_fmt=png)
...
![alt](https://mmbiz.qpic.cn/mmbiz_jpg/j3gficicyOvasF6pzMRVlOnbpMNic3XqHBP73n4OwGhKicGfVtib3Xgs5hibtaPEpicyOEs877gU0aBhn36U3SWNdwlIA/640?wx_fmt=jpeg)
...
![alt](https://mmbiz.qpic.cn/mmbiz_png/j3gficicyOvasF6pzMRVlOnbpMNic3XqHBP8icNRJOiakkHl0zLwThQ3icibZIgrDPwNAHGxdp52RkjFMFC58OGgDoEuw/640?wx_fmt=png)
...
![alt](https://mmbiz.qpic.cn/mmbiz_png/j3gficicyOvasF6pzMRVlOnbpMNic3XqHBPyWmaB0jibZgzaXBWzSFZ5wiac5Fvy7wO3GXVJLkZbIxjZicD3DjHfTgaw/640?wx_fmt=png)

下面两图演示的窗口的滑动情况，收到 36 的 ACK 后，窗口向后滑动 5 个 byte。

![alt](https://mmbiz.qpic.cn/mmbiz_png/j3gficicyOvasF6pzMRVlOnbpMNic3XqHBPHzNZKPofdmicDgC4CFuQQ9aE5ic619ia2wicicDls8BIibW34riaP4jQ8wUnw/640?wx_fmt=png)![alt](https://mmbiz.qpic.cn/mmbiz_png/j3gficicyOvasF6pzMRVlOnbpMNic3XqHBPDiahSGFpe73WSAGibZq9pejuLOTcHflGmhOT4S7WWutQNXzOePztqDlA/640?wx_fmt=png)

#### 疑问 2）的解决

由问题 1）我们知道，发送端的发送窗口是由接收端控制的。下图，展示了一个发送端是怎么受接收端控制的。

![alt](https://mmbiz.qpic.cn/mmbiz_png/j3gficicyOvasF6pzMRVlOnbpMNic3XqHBPpfalCJuBoemnU2RcgkhjPol7ibLnxb7xIhJmiciaZ0w7EOXK1Hq6OGxwg/640?wx_fmt=png)
...
![alt](https://mmbiz.qpic.cn/mmbiz_jpg/j3gficicyOvasF6pzMRVlOnbpMNic3XqHBPgap9ABwSzsmzD26f7yzO5vzH49kvlpATg742PJjibhKpTpfibFU8DuCg/640?wx_fmt=jpeg)
...
于是总耗时为 2_FS/(3_BW)完成了文件 FS 的传输，可以看出耗时减少为原来的 2/3 了，如果有 n 个节点，那么时间就是原来的 2/(n-1)，也就是加速比是 2/(n-1)，这就是加速的理论上限了吗？还没发挥最多能量的，相信大家已经看到分割文件的好处了，上面的文件分割粒度还是有点大，以至于，在第二阶段 [2] 传输过程中，节点 N1 无所事事。为了最大化发挥大家的作用，我们需要将 FS2,FS3,FS4 在进行分割，假设将它们都均分为 K 等份，这样就有 FS21,FS22…FS2K、FS31,FS32…FS3K、FS41,FS42…FS4K，一共 3K 个分段。于是下面就开始进行加速分发：
...
![alt](https://mmbiz.qpic.cn/mmbiz_png/j3gficicyOvasF6pzMRVlOnbpMNic3XqHBPTRBPdtgzicaatbwwBvHuiaIiag2WDkjISqnSp1SSdS9yoialghqvab6Wibg/640?wx_fmt=png)
...
[http://coolshell.cn/articles/11609.html](http://coolshell.cn/articles/11609.html)

[http://www.tcpipguide.com/free/t_TCPMessageSegmentFormat.htm](http://www.tcpipguide.com/free/t_TCPMessageSegmentFormat.htm)l
